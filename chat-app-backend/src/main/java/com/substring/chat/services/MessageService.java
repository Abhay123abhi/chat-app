...
public class MessageService {
    ...
    public MessageService(MongoTemplate mongo, ChatProperties properties) {
        this.mongo = mongo;
    ...
    public Message save(String roomId, MessageRequest request) {
        ...
        if (request == null || request.clientMessageId() == null
                ...
                || request.sender() == null || request.sender().isBlank() || request.sender().trim().length() > 50
                || request.content() == null || request.content().isBlank() || request.content().trim().length() > maxLength) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
                    "Invalid message: name max 50, text max " + maxLength + ", request ID required");
        }
        ...
        try {
            ...
            int maxMessages = Math.max(1, properties.getMaxMessagesPerRoom());
            Criteria roomAndQuota = new Criteria().andOperator(
            ...
            Room room = mongo.findAndModify(Query.query(roomAndQuota), roomUpdate,
                    FindAndModifyOptions.options().returnNew(true), Room.class);

            if (room == null) {
                boolean exists = mongo.exists(Query.query(Criteria.where("roomId").is(roomId)), Room.class);
                if (!exists) throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Room not found");
                throw new ResponseStatusException(HttpStatus.CONFLICT,
                        "Room has reached its " + maxMessages + " message limit");
            }
    ...
    public MessagePage history(String roomId, Long before, Long after, int limit) {
        validRoom(roomId);
        int maxPage = Math.max(1, properties.getMaxHistoryPageSize());
        if (limit < 1 || limit > maxPage || (before != null && after != null)
                || (before != null && before < 1) || (after != null && after < 0)) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
                    "Use one valid cursor and a limit from 1 to " + maxPage);
        }
        ...
        Query query = Query.query(criteria)
                .with(Sort.by(after == null ? Sort.Direction.DESC : Sort.Direction.ASC, "sequence"))
                .limit(limit + 1);

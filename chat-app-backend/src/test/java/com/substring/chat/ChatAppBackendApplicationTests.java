...
class ChatAppBackendApplicationTests {
    ...
    void setup() {
        ...
        service = new MessageService(mongo, new ChatProperties());
    }

    @Test
    void retryReturnsOriginalWithoutAllocatingOrInsertingAgain() {
        var original = message(4);
    ...
    void roomQuotaReturnsConflict() {
        ...
        when(mongo.findAndModify(any(Query.class), any(Update.class),
                any(FindAndModifyOptions.class), eq(Room.class))).thenReturn(null);
    ...
    void olderPageReturnsChronologicalResultsAndExclusiveCursor() {
        when(mongo.exists(any(Query.class), eq(Room.class))).thenReturn(true);
        when(mongo.find(any(Query.class), eq(Message.class)))
                .thenReturn(List.of(message(10), message(9), message(8)));
        ...
        assertEquals(11L, ((Document) captor.getValue().getQueryObject().get("sequence")).get("$lt"));
    }
    ...
    private static Message message(long sequence) {
